...
class SMCELoss(Loss):
    def forward(self, y, yhat):
        """Coût log soft max - cross-entropique
    ...
    def backward(self, y, yhat):
        """ Calculer le gradient du cout par rapport yhat
        """
        exp_sum_exp = np.exp(yhat)/np.sum(np.exp(yhat), axis = 1)[:, np.newaxis] # exp(yhat_k)/sum(exp(yhat_i))
        tmp_2 = np.where(y==1, -1, 0)
        return exp_sum_exp + tmp_2 # où y = 1 -> -1
...
class BCELoss(Loss):
    ...
    def backward(self, y, yhat):
        ...
        yhat = np.where(yhat == 1, 0.999999999, yhat) # smoothing
